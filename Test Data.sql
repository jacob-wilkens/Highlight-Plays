...
UPDATE TEST_DATA SET HOMEDESCRIPTION = N.HOMEDESCRIPTION
					,VISITORDESCRIPTION = N.VISITORDESCRIPTION
					FROM (
					SELECT GAME_ID
						, EVENTNUM
						, CASE WHEN Charindex('S. HILL', HOMEDESCRIPTION) > 0 THEN TRIM(REPLACE(HOMEDESCRIPTION, 'S. HILL', ''))
		 					ELSE HOMEDESCRIPTION
							END AS HOMEDESCRIPTION
						, CASE WHEN Charindex('S. HILL', VISITORDESCRIPTION) > 0 THEN TRIM(REPLACE(VISITORDESCRIPTION, 'S. HILL', ''))
		 					ELSE VISITORDESCRIPTION
							END AS VISITORDESCRIPTION
					FROM TEST_DATA 
					WHERE HOMEDESCRIPTION LIKE '%S. HILL%' OR VISITORDESCRIPTION LIKE '%S. HILL%'
					) 
					AS N WHERE N.GAME_ID = TEST_DATA.GAME_ID AND N.EVENTNUM = TEST_DATA.EVENTNUM
...
--FROM #FIFTH_PASS FP
--JOIN TEST_DATA PD ON PD.GAME_ID = FP.GAME_ID AND PD.EVENTNUM = FP.EVENTNUM
--WHERE FP.HOMEDESCRIPTION = '36'' 3PT Pullup Jump Shot'
